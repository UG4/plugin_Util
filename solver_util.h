...
namespace ug{
    namespace Util{
        ...
        SmartPtr<StdConvCheck<typename TAlgebra::vector_type>> CreateConvCheck(nlohmann::json &descriptor){
            ...
            UG_LOG("[solver_util.h] CreateConvCheck type = " << name << "\n");
            UG_LOG("[solver_util.h] CreateConvCheck iterations = " << iterations << "\n");
            UG_LOG("[solver_util.h] CreateConvCheck absolute = " << absolute << "\n");
            UG_LOG("[solver_util.h] CreateConvCheck reduction = " << reduction << "\n");
            UG_LOG("[solver_util.h] CreateConvCheck verbose = " << verbose << "\n");
            UG_LOG("[solver_util.h] CreateConvCheck suppress_unsuccessful = " << suppress_unsuccessful << "\n");
        ...
        void SetDebugWriter(SmartPtr<TSolver> obj,
            ...
            if (dbgDescIt != desc.end()){
                ...
                std::stringstream ss;
                ss << "SetDebugWriter: debug=" << (debug ? 1 : 0)
                   << " vtk=" << (vtk ? 1 : 0)
                   << " conn_viewer=" << (conn_viewer ? 1 : 0) << std::endl;
                UG_LOG(ss.str());
        ...
        CreateLinearSolver(nlohmann::json &solverDesc, SolverUtil<TDomain, TAlgebra> &solverutil){
            ...
            if (type == "linear"){
                UG_LOG("[solver_util.h] type solver is linear\n");
                // create linear
        ...
        CreateNewtonSolver(nlohmann::json &solverDesc, SolverUtil<TDomain, TAlgebra> &solverutil){
            ...
            if (type == "newton"){
                ...
                // line search
                nlohmann::json              lineSearchDesc = nullptr;
                std::optional<std::string>  lineSearchType;

                // defaults JSON (can be string, object or null)
                if (json_default_nonlinearSolver["newton"]["lineSearch"].is_string()){
                    lineSearchType = json_default_nonlinearSolver["newton"]["lineSearch"].get<std::string>();
                }
                else if (json_default_nonlinearSolver["newton"]["lineSearch"].is_object()){
                ...
                if (solverDesc.contains("lineSearch") && !solverDesc["lineSearch"].is_null()){
                    if (solverDesc["lineSearch"].is_string()){
                        // UG_LOG(">> LineSearchDesc is a string from solverDesc\n");
                        lineSearchType = solverDesc["lineSearch"].get<std::string>();
                    }
                    else if (solverDesc["lineSearch"].is_object()){
                ...
                bool have_valid_string = lineSearchType.has_value() && !lineSearchType->empty() && *lineSearchType != "none";
                bool have_valid_object = lineSearchDesc.is_object() &&
                                         lineSearchDesc.contains("type") &&
                                         lineSearchDesc["type"].is_string() &&
                                         lineSearchDesc["type"].get<std::string>() != "none";
                
                // Set line Search on newton Solver if valid string or object
                if (have_valid_string || have_valid_object){
                    if (have_valid_string && !lineSearchDesc.is_object()){
                        lineSearchDesc = nlohmann::json::object();
                        lineSearchDesc["type"] = *lineSearchType;
                    }
                    else if (have_valid_string && lineSearchDesc.is_object() && !lineSearchDesc.contains("type")){
                        lineSearchDesc["type"] = *lineSearchType;
                    }
        ...
        CreatePreconditioner(nlohmann::json &desc, SolverUtil<TDomain, TAlgebra> &solverutil){
            ...
            // TODO: precond.set_ordering_algorithm(ordering)
                ...
                if (solverutil.hasComponent("discretization")){
                    ...
                    UG_LOG("[solver_util.h] gmg: Discretization found!\n");
                ...
                SmartPtr<ILinearOperatorInverse<TVector>> skeletonSolver = CreateLinearSolver(skeletonSolverDesc, solverutil);
                SmartPtr<ISchurComplementInverse<TAlgebra>> schurInv = make_sp(new SchurInverseWithFullMatrix<TAlgebra>(skeletonSolver));
