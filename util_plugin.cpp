...
namespace ug
{
    namespace Util
    {
        ...
        void PluginSaysHello()
        {
            ...
            cout << "Hello, I'm your plugin on proc " << pcl::ProcRank() << "." << endl;
            pcl::SynchronizeProcesses();
#else
        ...
        struct Functionality
        {
            ...
            static void Domain(TRegistry &reg, string grp)
            {
                ...
                }
                {
                    ...
                    std::string grpname = std::string("CreateDomain");
                    reg.add_function(grpname.append(suffix),
                                     static_cast<SmartPtr<TDomain> (*)(const std::string &, int)>(&CreateDomain<TDomain>), grpname);
                }
                {
                    // This is more complicated, as the return type should be included in name.
                    std::string grpname = std::string("CreateDomain");
                    reg.add_function(grpname.append(suffix),
                                     static_cast<SmartPtr<TDomain> (*)(const std::string &, int, const std::vector<std::string> &, bool)>(&CreateDomain<TDomain>), grpname);
                }
            }
